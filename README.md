...
```text
dotnet publish -c Release -f net10.0 -r linux-x64
dotnet publish -c Release -f net10.0 -r osx-x64
...
```text
error MSB3030: Could not copy the file "obj\Release\net10.0\win-x64\Microsoft.Office.WopiValidator.dll"
...
The output will be placed in `src/WopiValidator/bin/Release/net10.0/linux-x64/publish` (replace `linux-x64`
...
`dotnet run --project ./src/WopiValidator/WopiValidator.csproj --framework net10.0 -- -t MyAccessToken -l 0 -w http://localhost:5000/wopi/files/<id> -e OfficeOnline -s`
